...
function DeltaBadge({ value }: { value: number }) {
  if (value === 0) {
    return (
      <span className="inline-flex items-center gap-1 rounded-full border border-line bg-slate-50 px-2.5 py-1 text-xs font-semibold text-slate-500">
        <Minus size={12} /> 0
      </span>
    );
  }
...
function IssueColumn({
  ...
  const toneClass = tone === "good" ? "text-emerald-600" : "text-rose-600";
  return (
    <div className="card">
      <h3 className={`mb-4 flex items-center gap-2 text-sm font-semibold ${toneClass}`}>
        <Icon size={16} /> {title} <span className="text-slate-400">({issues.length})</span>
      </h3>
      {issues.length === 0 ? (
          ...
          {issues.map((issue, idx) => (
            <li key={`${issue.file}-${issue.line}-${idx}`} className="flex items-start gap-3 py-3">
              ...
              <div className="min-w-0">
                <p className="text-sm text-slate-800">{issue.issue}</p>
...
export function ReportDiff({ diff }: { diff: ReportDiffResult }) {
  ...
  return (
    <div className="space-y-6">
      ...
      <section className="card animate-fade-up">
        <h3 className="mb-4 text-sm font-semibold text-slate-900">Category scores</h3>
        <div className="grid grid-cols-2 gap-3 sm:grid-cols-3">
          {CATEGORIES.map((c) => (
            <div
              key={c}
              className="flex items-center justify-between rounded-xl border border-line bg-slate-50 px-4 py-3"
            >
